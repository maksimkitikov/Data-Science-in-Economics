...
This repository contains every input, every line of code and every output behind
the data-driven blog post **["Beyond GDP — what really drives national
happiness?"](docs/index.html)**. The project is a self-contained, reproducible
pipeline that scrapes the World Happiness Report (WHR) website, downloads the
Report's published ranking spreadsheets, queries the World Bank API for
supplementary indicators, integrates the three sources in a relational database,
and finally fits both classical OLS and a modern causal-machine-learning model
(`CausalForestDML`) to investigate cross-country variation in life
satisfaction.

📍 **Final blog post:** [`docs/index.html`](docs/index.html) — also available
   as a Jupyter notebook (`blog.ipynb`) at the repository root.
...
`make all` walks through the eight numbered scripts in `src/`, regenerates the
SQLite database, every figure, the regression table, and the rendered blog
post. Subsequent `make` invocations only re-run steps whose inputs have
changed (this is the standard `make` behaviour, recommended by *Workflow,
Modelling & Webscraping.pdf* and Gentzkow & Shapiro 2014).
...
├── docs/
│   └── index.html                  ← rendered blog (GitHub Pages entry)
├── src/
...
`pystout` to a single LaTeX table — exactly as in Clarke's
`immigrantEffects.py`.
