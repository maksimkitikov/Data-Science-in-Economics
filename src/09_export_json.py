...
09_export_json.py
-----------------
Export the JSON files the static website (`docs/`) consumes, plus a single
`docs/data.js` bundle so the site can be opened straight from disk without
running a local web server.

Each Plotly chart on the site reads exactly one JSON file - the schema
mirrors the figures produced by 06_causal_forest.py and 07_descriptive_figures.py.

Course references followed here
    * Workflow, Modelling & Webscraping (Clarke, 2026), slide 8: workflow
      should "efficiently permit replication" - mirroring all chart inputs
      as JSON makes the website rebuildable without a Python kernel.
    * Coding for Economists (Turrell, 2023), §"Project Layout": tables and
      figures are derived artefacts, never edited by hand.
"""
import json
import os
...
# Causal-forest split-importance.  Persisted by 06_causal_forest.py; if for
# any reason the file is missing we fall back to the values reported in the
# original run so the site still renders.
