"""
The descriptive figures that open the blog post. There are five plots:

    1. Top-10 vs bottom-10 happiest countries (horizontal bar chart)
    2. GDP per capita vs Ladder, log axis, with a LOWESS smoother
    3. Variance decomposition for the top-15 happiest (stacked bars)
    4. WHR research output over time (chapters & mean reading time)
    5. Ladder time series, 2020-2024, for a handful of selected countries

Style choices (clean grids, country-code annotations, tight_layout, both
PDF and PNG output) follow `developmentMortality.py` from Damian's repo.
"""
import os
import numpy as np
import pandas as pd
...
# Each panel gets its own x-axis so the small differences within each group
# read clearly. The overall context (top ~7.8, bottom ~3-4) is communicated
# via the titles and the prose.
...
# GDP vs Ladder scatter, with LOWESS fit
...
# Stacked-bar variance decomposition for the top-15 happiest countries.
# We fit a tiny OLS of the Ladder on its six standard components, then break
# down each country's score into beta_k * X_k contributions. (We could read
# this off the WHR's "Explained by:" columns directly, but those are only
# present for some yearly files, so this works as a unified solution.)
...
# Wider canvas + legend pushed outside so it never covers any bar
...
# WHR research output over time (built from the scraped chapter table)
...
# Time series: WHR Ladder for a handful of countries 2020-2024
