"""
Assemble the blog post programmatically: build a notebook with nbformat,
execute every cell, and render it to a standalone HTML file in docs/notebook.html
as a technical companion to the main interactive site (docs/index.html). Doing
it from a script (rather than hand-editing .ipynb) means the rendered post
always matches the upstream pipeline.
"""
import os
...
HTML_PATH = DOCS + "notebook.html"   # main site lives in docs/index.html
...
md("""A few things to notice. The log-GDP coefficient falls from about 0.80
...
md("""The forest's average treatment effect lands very close to zero once we
...
md("""Internet penetration is the most important moderator, ahead of social
...
# Render the executed notebook to docs/notebook.html (technical companion)
